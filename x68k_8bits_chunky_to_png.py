# Convert X68000 8 bits (256 colors) paletted chunky pictures files into png pictures.
...
def convert_picture(png_name, pic_name, pal_name, width, height, off_pic, off_pal):
    ...
    for j in range(int(height)):
        for i in range(int(width)):
            final_pict[j][(i * 4)] = red[pict[(j * int(width)) + i]]
            final_pict[j][(i * 4) + 1] = green[pict[(j * int(width)) + i]]
            final_pict[j][(i * 4) + 2] = blue[pict[(j * int(width)) + i]]
...
def main():
    print('x68k_8bits_chunky_to_png by Franck "hitchhikr" Charlet.')
    if len(sys.argv) < 6:
        print("Usage: x68k_8bits_chunky_to_png <png picture file> <picture file> <palette file> <width> <height> [offset in pic file] [offset in pal file]")
        return
    if len(sys.argv) < 7:
        offset_pic = 0
    else:
