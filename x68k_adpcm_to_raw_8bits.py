# Convert X68000 adpcm samples files into raw 8 bits files.
...
def main():
    print('x68k_adpcm_to_raw_8bits by Franck "hitchhikr" Charlet.')
    if len(sys.argv) < 3:
        print("Usage: x68k_adpcm_to_raw_8bits <raw8 file> <adpcm file> [adpcm size] [offset in adpcm file]")
        return
    if len(sys.argv) < 5:
        offset_adpcm = 0
    else:
